<?php get_header(); ?>

<div id="page-title">Menu</div>

<div class="menu-container">

  <button class="menu-category" id="appetizer">Appetizer</button>
  <div class="menu-content" id="appetizer-content">

      <div class="item-title">Salad</div>

      <div class="items">
        <div class="item">
          <h4>Edamame</h4>
          <p>boiled and salted soybeans<br>5.50</p>
        </div>
        <div class="item">
          <h4>Spicy Edamame</h4>
          <p>pan-fried soybeans with Japanese spice mix<br>6.50</p>
        </div>
        <div class="item">
          <h4>Green Salad</h4>
          <p>served with house dressing sauce<br>4.95</p>
        </div>
        <div class="item">
          <h4>Seaweed Salad</h4>
          <p>seaweed marinated with sesame oil<br>5.95</p>
        </div>
        <div class="item">
          <h4>Veggie Sunomono</h4>
          <p>cucumber in vinegar sauce<br>5.50</p>
        </div>
        <div class="item">
          <h4>Seafood Sunomono </h4>
          <p>cucumber with assorted seafood in vinegar sauce<br>7.95</p>
        </div>
        <div class="item">
          <h4>Hawaiian Poke</h4>
          <p>ahi tuna, cucumber, onion with ponzu, sesame oil, spicy sauce & tobiko<br>11.95</p>
        </div>
      </div>


      <div class="item-title">Grilled</div>

      <div class="items">
        <div class="item">
          <h4>Baked Green Mussel</h4>
          <p>baked on half shell & spicy sauce - 5 pcs<br>7.50</p>
        </div>
        <div class="item">
          <h4>BBQ Albacore</h4>
          <p>grilled rare white tuna with special spicy BBQ sauce - 6 pcs<br>14.95</p>
        </div>
        <div class="item">
          <h4>Sea Steak</h4>
          <p>grilled ahi tuna with ponzu & special sauce - 8 pcs<br>14.95</p>
        </div>
        <div class="item">
          <h4>Chicken Teriyaki</h4>
          <p>grilled chicken with teriyaki sauce<br>9.95</p>
        </div>
        <div class="item">
          <h4>Hamachi Kama</h4>
          <p>grilled yellow tail cheek bone with ponzu sauce<br>10.50</p>
        </div>
      </div>


      <div class="item-title">Deep-Fried</div>

      <div class="items">
        <div class="item">
          <h4>Agedashi Tofu</h4>
          <p>deep-fried tofu served with dried tuna, ponzu sauce<br>7.95</p>
        </div>
        <div class="item">
          <h4>Calamari Tempura</h4>
          <p>breaded deep-fried calamari ring<br>8.50</p>
        </div>
        <div class="item">
          <h4>Croquette</h4>
          <p>deep-fried potato & vegetables – 3pcs<br>6.50</p>
        </div>
        <div class="item">
          <h4>Creamy Jalapeno </h4>
          <p>deep-fried jalapeno, cream cheese, spicy tuna special sauce<br>6.95</p>
        </div>
        <div class="item">
          <h4>Gyoza</h4>
          <p>Deep-fried or steamed chicken and vegetable pot-stickers - 7 pcs<br>7.50</p>
        </div>
        <div class="item">
          <h4>Vegetable Tempura</h4>
          <p>assorted seasonal vegetables<br>7.50</p>
        </div>
        <div class="item">
          <h4>Shrimp Tempura</h4>
          <p>deep-fried shrimp - 4 pcs<br>9.95</p>
        </div>
        <div class="item">
          <h4>Mixed Tempura</h4>
          <p>shrimp - 2 pcs, vegetables - 5 pcs<br>8.95</p>
        </div>
        <div class="item">
          <h4>Honey Spice Chicken</h4>
          <p>pan fried chicken, honey spice sauce<br>9.95</p>
        </div>
        <div class="item">
          <h4>Sesame Chicken</h4>
          <p>tempura fried chicken, teriyaki sauce<br>9.95</p>
        </div>
      </div>


      <div class="item-title">Raw Fish</div>

      <div class="items">
        <div class="item">
          <h4>Kuma Oyster</h4>
          <p>half shell oyster with spicy ponzu sauce, lemon - 4 pcs<br>9.95</p>
        </div>
        <div class="item">
          <h4>Pepper Fin</h4>
          <p>white tuna with jalapeno, spicy ponzu sauce and sesame oil<br>14.95</p>
        </div>
        <div class="item">
          <h4>7 Pcs Sashimi</h4>
          <p>choice of one - salmon, tuna, yellowtail - 7 pcs<br>14.50</p>
        </div>
        <div class="item">
          <h4>Assorted Sashimi</h4>
          <p>chef's choice of today's raw fish - 12 pcs<br>19.95</p>
        </div>
      </div>
    </div>

  <button class="menu-category" id="nigiri">Nigiri</button>
  <div class="menu-content" id="nigiri-content">

    <div class="item-title">Nigiri</div>
    <p>* Extra order: Uzura (Quail Egg).....1.50 *</p>
    <div class="items">
      <div class="item">
          <h4>Albacore</h4>
          <p>white tuna<br>5.95</p>
      </div>
      <div class="item">
        <h4>Ama-Ebi</h4>
        <p>sweet shrimp<br>7.50</p>
      </div>
      <div class="item">
        <h4>Ebi</h4>
        <p>cooked shrimp<br>4.50</p>
      </div>
      <div class="item">
        <h4>Escolar</h4>
        <p>escolar<br>5.95</p>
      </div>
      <div class="item">
        <h4>Hamachi</h4>
        <p>yellow tail<br>5.95</p>
      </div>
      <div class="item">
        <h4>Hokkigai</h4>
        <p>surf clam<br>5.95</p>
      </div>
      <div class="item">
        <h4>Hotate</h4>
        <p>scallop<br>5.95</p>
      </div>
      <div class="item">
        <h4>Ika</h4>
        <p>squid<br>4.50</p>
      </div>
      <div class="item">
        <h4>Ikura</h4>
        <p>salmon roe<br>5.50</p>
      </div>
      <div class="item">
        <h4>Inari</h4>
        <p>tofu wrap<br>4.50</p>
      </div>
      <div class="item">
        <h4>Kani</h4>
        <p>snow crab<br>7.95</p>
      </div>
      <div class="item">
        <h4>Maguro</h4>
        <p>tuna<br>5.95</p>
      </div>
      <div class="item">
        <h4>Masago</h4>
        <p>smelt roe<br>4.95</p>
      </div>
      <div class="item">
        <h4>Saba</h4>
        <p>mackerel<br>4.95</p>
      </div>
      <div class="item">
        <h4>Sake</h4>
        <p>salmon<br>5.95</p>
      </div>
      <div class="item">
        <h4>Spicy Hotate</h4>
        <p>spicy scallop<br>5.95</p>
      </div>
      <div class="item">
        <h4>Tako</h4>
        <p>octopus<br>4.95</p>
      </div>
      <div class="item">
        <h4>Tamago</h4>
        <p>sweet egg<br>4.50</p>
      </div>
      <div class="item">
        <h4>Tobiko</h4>
        <p>flying fish roe<br>5.95</p>
      </div>
      <div class="item">
        <h4>Unagi</h4>
        <p>fresh water eel<br>5.95</p>
      </div>

    </div>


  </div>

  <button class="menu-category" id="entree">Entrée</button>
  <div class="menu-content" id="entree-content">
    <div class="item-title">KUMA BENTO BOX</div>
    <div class="bento-description">Lunch: Before 4 pm &nbsp; 11.95<br>Dinner: After 4 pm &nbsp; 16.95<br><span style="font-size: 20px">* Served with soup, salad & rice *<br>* Duplicate choices charge extra $ 1*<br>* No substitution on bento box *</span></div>
    <div class="bento-items">
      <div class="item">Beef Teriyaki</div>
      <div class="item">Chicken Teriyaki</div>
      <div class="item">Salmon Teriyaki</div>
      <div class="item">Fish Katsu</div>
      <div class="item">Ton Katsu</div>
      <div class="item">Chicken Katsu</div>
      <div class="item">California Roll</div>
      <div class="item">Tuna Sashimi 4ps</div>
      <div class="item">Saba Shioyaki</div>
      <div class="item">Sake Nigiri 2pcs</div>
      <div class="item">Sesame Chicken</div>
      <div class="item">Gyoza - 4pc</div>
      <div class="item">Croquette – 2pcs</div>
      <div class="item">Mixed Tempura (shrimp - 1pc , vegetable - 5pcs)</div>
    </div>

    <div class="item-title">NOODLE SOUP</div>
    <div class="items">
      <div class="item">
        <h4>Veggie Udon</h4>
        <p>a thick noodle soup with vegetable<br>L 10.95 | D 13.95</p>
      </div>

      <div class="item">
        <h4>Beef Udon</h4>
        <p>a thick noodle soup with beef & vegetable<br>L 12.95 | D 15.95</p>
      </div>

      <div class="item">
        <h4>Nabe Udon</h4>
        <p>a thick noodle soup with shrimp tempura, chicken, vegetable<br>L 12.95 | D 15.95</p>
      </div>

      <div class="item">
        <h4>Tempura Udon</h4>
        <p>a thick noodle soup with vegetable & mixed tempura on the side<br>L 12.95 | D 15.95</p>
      </div>

      <div class="item">
        <h4>Seafood Udon</h4>
        <p>a thick noodle soup with assorted seafood & vegetable<br>L 12.95 | D 15.95</p>
      </div>
    </div>

    <div class="item-title">KUMA SINGLE ENTRÉE</div>
    <div class="items">
      <div class="item">
        <h4>Chicken Teriyaki</h4>
        <p>Grilled chicken with teriyaki sauce<br>L 11.95 | D 16.95</p>
      </div>

      <div class="item">
        <h4>Salmon Teriyaki</h4>
        <p>Grilled salmon steak with teriyaki sauce<br>L 12.95 | D 17.95</p>
      </div>

      <div class="item">
        <h4>Beef Teriyaki</h4>
        <p>Grilled beef steak with teriyaki sauce<br>L 12.95 | D 17.95</p>
      </div>

      <div class="item">
        <h4>Saba Shioyaki</h4>
        <p>lightly salted and Grilled mackerel fillet<br>L 11.95 | D 16.95</p>
      </div>

      <div class="item">
        <h4>Vegetable Tempura</h4>
        <p>Assorted seasonal vegetable | Lunch - 7pcs | Dinner – 9pcs<br>L 10.95 | D 15.95</p>
      </div>

      <div class="item">
        <h4>Shrimp Tempura</h4>
        <p>Shrimp - Lunch - 5pcs, Dinner - 7 pcs<br>L 12.95 | D 17.95</p>
      </div>

      <div class="item">
        <h4>Mixed Tempura</h4>
        <p>Shrimp - Lunch - 3 pcs, Dinner – 4 pcs | Vegetable – Lunch - 5 pcs, Dinner – 7 pcs<br>L 12.95 | D 17.95</p>
      </div>

      <div class="item">
        <h4>Chicken Katsu</h4>
        <p>Panko fried chicken breast cutlets with katsu sauce<br>L 11.95 | D 16.95</p>
      </div>

      <div class="item">
        <h4>Ton Katsu</h4>
        <p>Panko fried pork loin cutlets with katsu sauce<br>L 11.95 | D 16.95</p>
      </div>

      <div class="item">
        <h4>Sesame Chicken</h4>
        <p>Tempura fried chicken breast with teriyaki sesame sauce<br>L 11.95 | D 16.95</p>
      </div>

    </div>

    <div class="item-title">Bowl Special</div>
    <div class="items">
      <div class="item">
        <h4>Ton Katsu</h4>
        <p>Panko fried Pork loin with katsu sauce over rice<br>L 9.95 | D 12.95</p>
      </div>
      <div class="item">
        <h4>Chicken Katsu</h4>
        <p>Panko fried chicken breast cutlets with katsu sauce over rice<br>L 9.95 | D 12.95</p>
      </div>
      <div class="item">
        <h4>Chicken Teriyaki</h4>
        <p>Grilled chicken with teriyaki sauce over rice<br>L 9.95 | D 12.95</p>
      </div>
      <div class="item">
        <h4>Sesame Chicken</h4>
        <p>Tempura fried chicken breast with teriyaki sesame sauce over rice<br>L 9.95 | D 12.95</p>
      </div>
    </div>
  </div>

  <button class="menu-category" id="kids">Kids' Menu</button>
  <div class="menu-content" id="kids-content">
    <div class="item-title"></div>
    <div class="items">
      <div class="item">
        <h4>Teddy Bear 1</h4>
        <p>Chicken teriyaki or Sesame chicken, California roll – 4pcs, gyoza - 2pcs<br>9.95</p>
      </div>
      <div class="item">
        <h4>Teddy Bear 2</h4>
        <p>Mixed tempura, California roll – 4pcs, gyoza - 2pcs<br>9.95</p>
      </div>
      <div class="item">
        <h4>Teddy Bear 3</h4>
        <p>Chicken Katsu or Ton Katsu, California – 4pcs, gyoza - 2pcs<br>9.95</p>
      </div>
    </div>
  </div>

  <button class="menu-category" id="sns">Sushi & Sashimi Combo</button>
  <div class="menu-content" id="sns-content">
    <div class="item-title">Sushi & Sashimi Combination</div>
    <p>Served with soup & salad<p>
    <div class="items">
      <div class="item">
        <h4>Sushi-5</h4>
        <p>5 pcs chef’s choice nigiri sushi & california roll<br>16.50</p>
      </div>

      <div class="item">
        <h4>Sushi-7</h4>
        <p>7 pcs chef’s choice nigiri sushi & california roll<br>20.50</p>
      </div>

      <div class="item">
        <h4>Sushi-9</h4>
        <p>9 pcs chef’s choice nigiri sushi & california roll<br>24.50</p>
      </div>

      <div class="item">
        <h4>Sushi & Sashimi - Lunch</h4>
        <p>3 pcs - sushi, 3 pcs - sashimi & California roll<br>15.50</p>
      </div>

      <div class="item">
        <h4>Sushi & Sashimi - Dinner</h4>
        <p>5 pcs- sushi, 4 pcs- sashimi & california roll<br>20.95</p>
      </div>

      <div class="item">
        <h4>Chirashi Lunch</h4>
        <p>12 pcs chef’s choice of sliced raw fish & served with sushi rice<br>20.95</p>
      </div>

      <div class="item">
        <h4>Chirashi Dinner</h4>
        <p>16 pcs chef’s choice of sliced raw fish & served with sushi rice<br>25.95</p>
      </div>

      <div class="item">
        <h4>Sashimi Lunch</h4>
        <p>12 pcs chef’s choice of sliced raw fish & served with steamed rice<br>20.95</p>
      </div>

      <div class="item">
        <h4>Sashimi Dinner</h4>
        <p>16 pcs chef’s choice of sliced raw fish & served with steamed rice<br>25.95</p>
      </div>

      <div class="item">
        <h4>Sashimi Deluxe</h4>
        <p>25 pcs chef’s choice of sliced raw fish & served with steamed rice<br>39.95</p>
      </div>
    </div>
  </div>

  <button class="menu-category" id="rolls">Rolls</button>
  <div class="menu-content" id="rolls-content">
    <div class="discount">Every Roll <span style="color: #bf1a04; font-weight: bold;">50% OFF!</span></div>

    <div class="item-title">Vegetable Roll</div>
    <div class="items">
      <div class="item">
        <h4>Asparagus Roll</h4>
        <p>asparagus<br>8.50</p>
      </div>

      <div class="item">
        <h4>Avokyu Roll</h4>
        <p>Avocado, cucumber<br>8.99</p>
      </div>

      <div class="item">
        <h4>Avocado Make</h4>
        <p>avocado<br>8.50</p>
      </div>

      <div class="item">
        <h4>Futomaki</h4>
        <p>egg, cucumber, oshinko, kampyo, yamagobo<br>8.50</p>
      </div>

      <div class="item">
        <h4>Kampyo Make</h4>
        <p>squash<br>8.50</p>
      </div>

      <div class="item">
        <h4>Kappa Maki</h4>
        <p>cucumber<br>8.50</p>
      </div>

      <div class="item">
        <h4>Oshinko Make</h4>
        <p>pickled radish<br>8.50</p>
      </div>

      <div class="item">
        <h4>Texas Roll</h4>
        <p>deep-fried jalapeno, avocado, cream cheese<br>9.99</p>
      </div>
    </div>

    <div class="item-title">Beginner's Roll</div>
    <div class="items">
      <div class="item">
        <h4>Alaskan Roll</h4>
        <p>salmon, avocado<br>9.99</p>
      </div>

      <div class="item">
        <h4>California Roll</h4>
        <p>crab meat, avocado<br>8.99</p>
      </div>

      <div class="item">
        <h4>Ebi Maki</h4>
        <p>cooked shrimp<br>8.99</p>
      </div>

      <div class="item">
        <h4>Negi Hama</h4>
        <p>yellowtail, green onion<br>8.99</p>
      </div>

      <div class="item">
        <h4>New York Roll</h4>
        <p>shrimp, avocado<br>9.99</p>
      </div>

      <div class="item">
        <h4>Ocean Roll</h4>
        <p>yellowtail, avocado<br>9.99</p>
      </div>

      <div class="item">
        <h4>Pacific Roll</h4>
        <p>tuna, avocado<br>9.99</p>
      </div>

      <div class="item">
        <h4>Philly Roll</h4>
        <p>salmon, cream cheese, avocado<br>9.99</p>
      </div>

      <div class="item">
        <h4>Rock'N'Roll</h4>
        <p>eel, avocado<br>9.99</p>
      </div>

      <div class="item">
        <h4>Sake Maki</h4>
        <p>salmon<br>8.99</p>
      </div>

      <div class="item">
        <h4>Salmon Skin Roll</h4>
        <p>salmon skin, cucumber<br>9.99</p>
      </div>

      <div class="item">
        <h4>Spicy California</h4>
        <p>spicy crabmeat, avocado<br>8.99</p>
      </div>

      <div class="item">
        <h4>Spicy Tuna Roll</h4>
        <p>spicy tuna, cucumber<br>9.99</p>
      </div>

      <div class="item">
        <h4>Tekka Maki</h4>
        <p>tuna<br>8.99</p>
      </div>
    </div>

    <div class="item-title">Temaki - Hand Roll</div>
    <div class="items">
      <div class="item">
        <h4>California Hand Roll </h4>
        <p>crab meat, avocado<br></p>
      </div>

      <div class="item">
        <h4>Tempura Hand Roll</h4>
        <p>shrimp tempura, avocado, crabmeat<br>8.50</p>
      </div>

      <div class="item">
        <h4>Spicy Tuna Hand Roll</h4>
        <p>spicy tuna, cucumber<br>8.99</p>
      </div>

      <div class="item">
        <h4>Unagi Hand Roll</h4>
        <p>eel, cucumber<br>8.99</p>
      </div>

      <div class="item">
        <h4>Fish Hand Roll</h4>
        <p>your choice of salmon, tuna, albacore, or yellow tail<br>8.99</p>
      </div>

      <div class="item">
        <h4>Chop Hotate Hand Roll</h4>
        <p>Spicy scallop<br>8.99</p>
      </div>

      <div class="item">
        <h4>Salmon Skin Hand Roll</h4>
        <p>salmon skin, cucumber<br>8.99</p>
      </div>

      <div class="item">
        <h4>Spider Hand Roll</h4>
        <p>Deep-fried softshell crab<br>8.99</p>
      </div>

      <div class="item">
        <h4>ZigZag Hand Roll</h4>
        <p>deep-friedsoftshell crab,soy wrap<br>8.99</p>
      </div>
    </div>
  </div>

  <button class="menu-category" id="ksr">Kuma Special Rolls</button>
  <div class="menu-content" id="ksr-content">
    <div class="discount">Every Roll <span style="color: #bf1a04; font-weight: bold;">50% OFF!</span></div>
    <div class="item-title">Non-Raw</div>
    <div class="items">
      <div class="item">
        <h4>Fair Way</h4>
        <p>Deep-fried shrimp | avocado, masago | special sauce<br>13.99</p>
      </div>

      <div class="item">
        <h4>Golden Gate</h4>
        <p>Deep-fried shrimp | crab meat, masago | special sauce<br>14.99</p>
      </div>

      <div class="item">
        <h4>Sesame Chicken Roll</h4>
        <p>crabmeat, avocado, ceram cheese | sesame chicken | unagi sauce<br>14.99</p>
      </div>

      <div class="item">
        <h4>Red Bull </h4>
        <p>deep-fried Shrimp, cream cheese | spicy crab meat, masago | special sauce | Soy wrap<br>15.99</p>
      </div>

      <div class="item">
        <h4>Spider</h4>
        <p>deep-fried soft shell crab, cucumber, avocado, crabmeat | masago | unagi sauce | 5pcs<br>14.50</p>
      </div>

      <div class="item">
        <h4>Tempura Crunchy</h4>
        <p>deep-friedshrimp, avocado, crab meat, crunchy |unagi sauce | 5pcs<br>13.99</p>
      </div>

      <div class="item">
        <h4>Tempura Roll</h4>
        <p>deep-fried shrimp, avocado, cucumber, crabmeat | 5pcs<br>11.99</p>
      </div>

      <div class="item">
        <h4>Veggie Tempura Roll</h4>
        <p>deep-fried- carrot, zucchini, asparagus | unagi sauce- 5pcs<br>11.50</p>
      </div>

      <div class="item">
        <h4>Yummy Yummy</h4>
        <p>deep-fried shrimp, avocado, cucumber, cream cheese, crab meat, masago | 5pcs<br>13.50</p>
      </div>
    </div>

    <div class="item-title">Eel</div>
    <div class="items">

      <div class="item">
        <h4>Blue Moon</h4>
        <p>deep-fried shrimp, orange | unagi, crab, masago | special sauce<br>17.50</p>
      </div>

      <div class="item">
        <h4>Caterpillar</h4>
        <p>Eel, cucumber | avocado | unagi sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Dragon</h4>
        <p>deep-fried shrimp, crabmeat | eel, masago | special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Hwy 50 </h4>
        <p>crabmeat, avocado | eel, cooked shrimp, Masago | special sacue<br>15.99</p>
      </div>

      <div class="item">
        <h4>Spicy Johny </h4>
        <p>deep-fried shrimp, spicy tuna | eel, masago | special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Spicy crunchy</h4>
        <p>Spicy tuna, eel, seaweed salad, cucumber | crunchy, unagi sauce | 5pcs <br>14.50</p>
      </div>

      <div class="item">
        <h4>Lake Tahoe</h4>
        <p>deep-fried shrimp, cream cheese, cucumber | unagi, masago | special sauce<br>16.50</p>
      </div>
    </div>

    <div class="item-title">Raw</div>
    <div class="items">
      <div class="item">
        <h4>49er</h4>
        <p>deep-friedshrimp, cream cheese, cucumber | salmon, lemon, masago | special sauce<br>15.50</p>
      </div>

      <div class="item">
        <h4>911</h4>
        <p>spicy tuna, cucumber | tuna, masago | spicy & special sauce<br>15.50</p>
      </div>

      <div class="item">
        <h4>Arigato</h4>
        <p>deep-friedShrimp | salmon, tuna, masago | special sauce<br>14.99</p>
      </div>

      <div class="item">
        <h4>Cherry Blossom</h4>
        <p>salmon, avocado | tuna, masago | special sauce<br>15.99</p>
      </div>

      <div class="item">
        <h4>Crazy Horse</h4>
        <p>salmon, tuna, yellowtail, avocado, masago | special sauce - 5pcs<br>15.99</p>
      </div>

      <div class="item">
        <h4>Folsom Lake</h4>
        <p>deep-friedshrimp, spicy tuna | salmon, masago | special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Happy</h4>
        <p>deep-friedshrimp | tuna, yellowtail, masago | special sauce<br>15.99</p>
      </div>

      <div class="item">
        <h4>Kamikaze</h4>
        <p>spicy crab, jalapeno, cream cheese | cooked shrimp, salmon, masago | special sauce<br>17.50</p>
      </div>

      <div class="item">
        <h4>Kevala</h4>
        <p>deep-fried shrimp, spicy tuna, crabmeat | scallop, masago, special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Kiss Me</h4>
        <p>Crab meat, avocado | salmon, masago | special sauce<br>15.50</p>
      </div>

      <div class="item">
        <h4>Kuma</h4>
        <p>deep-friedshrimp | avocado, sweet shrimp & 5 kinds of raw fish masago | special sauce<br>18.50</p>
      </div>

      <div class="item">
        <h4>Las Vegas</h4>
        <p>deep-friedAsparagus, crabmeat, cream cheese | tuna, salmon, masago, special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Mafia</h4>
        <p>deep-friedshrimp, crabmeat, cream cheese | scallop, salmon, masago | special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Orange Blossom</h4>
        <p>Tuna, avocado | salmon, masago | special sauce<br>15.99</p>
      </div>

      <div class="item">
        <h4>Pier 39</h4>
        <p>spicy tuna, cucumber | yellowtail, masago, spicy & special sauce<br>15.50</p>
      </div>

      <div class="item">
        <h4>Pink Lady</h4>
        <p>deep-fried shrimp, crab meat | tuna, masago | special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Rainbow</h4>
        <p>Crab meat, avocado | 5 kinds of raw fish<br>17.50</p>
      </div>

      <div class="item">
        <h4>Red Dragon</h4>
        <p>crabmeat, avocado | spicy tuna, cooked shrimp<br>13.50</p>
      </div>

      <div class="item">
        <h4>Red Hawk</h4>
        <p>deep-fried shrimp, crab meat | scallop, masago | special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Samurai</h4>
        <p>deep-fried asparagus, spicy crab, spicy tuna | salmon, white tuna, masago | special sauce<br>16.50</p>
      </div>

      <div class="item">
        <h4>Sunrise</h4>
        <p>spicy crab, Salmon, tuna, yellowtail, avo | spicy& special sauce - 5pcs<br>16.99</p>
      </div>

      <div class="item">
        <h4>Valentine Roll</h4>
        <p>deep-fried shrimp, crab meat, cilantro, lemon | tuna, salmon, g onion | special sauce<br>17.99</p>
      </div>

      <div class="item">
        <h4>Viagra</h4>
        <p>Spicy tuna, spicy crab meat, | tuna, salmon, yellowtail, masago | spicy & special sauce<br>16.99</p>
      </div>
    </div>

    <div class="item-title">Seared</div>
    <div class="items">
      <div class="item">
        <h4>Aloha</h4>
        <p>Spicy tuna, spicy crab meat, cucumber | seared albacore, jalapeno, masago | special sauce<br>15.99</p>
      </div>

      <div class="item">
        <h4>Bad Boys</h4>
        <p>deep-fried asparagus, crab meat | seared albacore, tuna, deep-friedjalapeno | unagi sauce<br>15.50</p>
      </div>

      <div class="item">
        <h4>Charisma</h4>
        <p>deep-fried shrimp | spicy crab, seared tuna, albacore, masago | special sauce<br>15.99</p>
      </div>

      <div class="item">
        <h4>El Dorado Hills</h4>
        <p>deep-fried soft shell crab, crab meat | salmon, seared tuna, masago | special sauce<br>17.99</p>
      </div>

      <div class="item">
        <h4>Lucky7</h4>
        <p>deep-fried shrimp, crabmeat, spicy tuna | seared salmon,  masago | special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Sakura</h4>
        <p>Spicy tuna, spicy crab meat, jalapeno | seared tuna, cooked shrimp, masago | special sauce<br>16.99</p>
      </div>

      <div class="item">
        <h4>Serrano</h4>
        <p>soft shell crab, crabmeat | seared salmon, masago | special sauce<br>17.50</p>
      </div>

      <div class="item">
        <h4>White Rock</h4>
        <p>deep-fried asparagus, spicy crab | seared white tuna, deep-fried onion | special sauce<br>15.50</p>
      </div>

      <div class="item">
        <h4>X-File</h4>
        <p>deep-fried shrimp, spicy tuna | 5 kinds of seared fish, masago | chilli pepper, unagi sauce<br>17.99</p>
      </div>

      <div class="item">
        <h4>Yosemite</h4>
        <p>deep-friedShrimp, spicy crab | seared tuna, deep-fried carrot | unagi sauce<br>15.50</p>
      </div>
    </div>

    <div class="item-title">Deep-fried</div>
    <div class="items">
      <div class="item">
        <h4>Golden CA</h4>
        <p>Crab meat, avocado | deep-fried, unagi sauce<br>11.99</p>
      </div>

      <div class="item">
        <h4>Golden Spicy</h4>
        <p>Spicy tuna, cream cheese, cucumber | deep-fried, spicy & unagi sauce<br>11.99</p>
      </div>

      <div class="item">
        <h4>Ninja</h4>
        <p>Spicy tuna, albacore, eel, cream cheese | deep-fried, masago, spicy & special sauce - 5pcs<br>15.99</p>
      </div>

      <div class="item">
        <h4>Oh! Yes</h4>
        <p>Salmon, tuna, yellowtail, avocado | deep-fried, unagi sauce - 5pcs<br>15.99</p>
      </div>
    </div>

    <div class="item-title">Baked</div>
    <div class="items">
      <div class="item">
        <h4>Lion King </h4>
        <p>Crab meat, avocado | salmon, special sauce & masago added after cooking - 6pcs<br>14.50</p>
      </div>

      <div class="item">
        <h4>Volcanic Scallop</h4>
        <p>Crab meat, avocado | scallop, special sauce & masago added after cooking - 6pcs<br>14.50</p>
      </div>
    </div>

    <div id="roll-extra">
      <h5>Would you like to add toppings?</h5>
      <div class="extras">
        <div class="extra">
          Masago | 1.00
        </div>

        <div class="extra">
          Cucumber | 1.00
        </div>

        <div class="extra">
          Crabmeat | 1.00
        </div>

        <div class="extra">
          Japapeno | 1.00
        </div>

        <div class="extra">
          Soy Wrap | 1.50
        </div>

        <div class="extra">
          Cream Cheese | 1.00
        </div>

        <div class="extra">
          Spicy Tuna | 1.50
        </div>

        <div class="extra">
          Extra Fish | 2.50
        </div>

        <div class="extra">
          Real Crab | 2.50
        </div>

        <div class="extra">
          Crunchy | 1.00
        </div>

        <div class="extra">
          Avocado | 1.00
        </div>

        <div class="extra">
          Deep-fried Shrimp | 1.50
        </div>

        <div class="extra">
          Spicy Crabmeat | 1.00
        </div>

        <div class="extra">
          Deep-fried Asparagus | 1.00
        </div>

        <div class="extra">
          Cooked Shrimp | 1.50
        </div>

        <div class="extra">
          Deep-fry your roll | 1.00
        </div>
      </div>
    </div>
  </div>

  <button class="menu-category" id="sides">Sides</button>
  <div class="menu-content" id="sides-content">
    <div class="item-title"></div>
    <div class="items">
      <div class="item">
        <h4>Miso Soup</h4>
        <p>2.00</p>
      </div>

      <div class="item">
        <h4>Bowl of Steamed Rice</h4>
        <p>2.00</p>
      </div>

      <div class="item">
        <h4>Bowl of Sushi Rice</h4>
        <p>2.00</p>
      </div>
    </div>
  </div>

  <button class="menu-category" id="alcohol">Alcoholic Beverage</button>
  <div class="menu-content" id="alcohol-content">
    <div class="item-title"></div>
    <div class="items">
      <div class="item">
        <h4>Beer Bottle 12oz</h4>
        <p>Sapporo, Asahi, Kirin, Bud Light, Corona, Blue moon<br>S 3.95 | L 6.50</p>
      </div>

      <div class="item">
        <h4>Draft Beer</h4>
        <p>Sapporo<br>12oz: 3.50 | 22oz: 6.50 | Pitcher: 15.95</p>
      </div>

      <div class="item">
        <h4>Wine</h4>
        <p>Red, White<br>Glass: 6.50</p>
      </div>

      <div class="item">
        <h4>Cold Sake</h4>
        <p>Premium Ginjo, Karatamba<br>13.95</p>
      </div>

      <div class="item">
        <h4>Nigori Sake</h4>
        <p>Unfiltered Sake<br>9.95</p>
      </div>

      <div class="item">
        <h4>Hot Sake</h4>
        <p>Sho chiku Bai<br>S: 3.50 | L: 6.50</p>
      </div>

      <div class="item">
        <h4>Sake Bomb</h4>
        <p>Sake & Sapporo Draft Beer<br>3.50</p>
      </div>

      <div class="item">
        <h4>Oyster Shooter</h4>
        <p>3.95</p>
      </div>
    </div>

    <div class="corkage">*** Corkage Fee - 10.00 ***</div>
  </div>

  <button class="menu-category" id="bnd">Beverage & Dessert</button>
  <div class="menu-content" id="bnd-content">
    <div class="item-title"></div>
    <div class="items">
      <div class="item">
        <h4>Soda</h4>
        <p>Coke, Diet Coke, Mr.pibb, Lemonade, Fanta, Sprite<br>2.50</p>
      </div>

      <div class="item">
        <h4>Ramune</h4>
        <p>Japanese marble soda<br>2.99</p>
      </div>

      <div class="item">
        <h4>Iced Tea</h4>
        <p>2.50</p>
      </div>

      <div class="item">
        <h4>Green Iced Tea</h4>
        <p>2.50</p>
      </div>

      <div class="item">
        <h4>Hot Tea</h4>
        <p>Genmai<br>1.00</p>
      </div>

      <div class="item">
        <h4>Mochi Ice cream</h4>
        <p>strawberry, chocolate, mango<br>3.99</p>
      </div>
    </div>

  </div>

</div>

<?php get_footer(); ?>
